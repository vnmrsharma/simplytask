...
import { ChevronLeft, ChevronRight, Calendar, TrendingUp, Target, Clock, CheckCircle, AlertTriangle, BarChart3, PieChart, Sparkles, Loader2, Minus, Award, Activity, ArrowUp } from 'lucide-react';
...
export const ReportsDashboard: React.FC<ReportsDashboardProps> = ({ tasks }) => {
  ...
  const handleError = (error: Error, errorInfo?: any) => {
    // console.error('ReportsDashboard Error:', error, errorInfo);
    setHasError(true);
    setErrorMessage(error.message || 'An unexpected error occurred in the Reports section.');
  };

  // Wrap the entire component in try-catch
  if (hasError) {
    return (
      <div className="max-w-7xl mx-auto p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <AlertTriangle className="h-12 w-12 text-red-600 mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-red-800 mb-2">Reports Error</h2>
  ...
  const safeReport = useMemo(() => {
    try {
      ...
      switch (selectedPeriod) {
        case 'daily':
          return generateDailyReport(tasks, selectedDate);
        case 'weekly':
          try {
            return generateWeeklyReport(tasks, selectedDate);
          } catch (error) {
            // console.error('Error generating weekly report:', error);
            handleError(new Error('Failed to generate weekly report'));
            return null;
          }
        case 'monthly':
          try {
            return generateMonthlyReport(tasks, selectedDate);
          } catch (error) {
            // console.error('Error generating monthly report:', error);
            handleError(new Error('Failed to generate monthly report'));
            return null;
          }
        default:
          return generateDailyReport(tasks, selectedDate);
  ...
  const handleSummarizeAI = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`AI Summary API failed: ${response.status} ${response.statusText}`);
      }
      const data = await response.json();
      setAiSummary(data.summary || 'Summary generated successfully');
    } catch (err: any) {
      // console.error('AI Summary error:', err);
      setAiError(err.message || 'Error generating summary. Please try again later.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleWeeklySummarizeAI = async () => {
    ...
    try {
      ...
      const data = await response.json();
      setWeeklyAiSummary(data.summary || 'Summary generated successfully');
    } catch (err: any) {
      // console.error('Weekly AI Summary error:', err);
      setWeeklyAiError(err.message || 'Error generating summary. Please try again later.');
    } finally {
  ...
  const handleMonthlySummarizeAI = async () => {
    ...
    try {
      ...
      const data = await response.json();
      setMonthlyAiSummary(data.summary || 'Summary generated successfully');
    } catch (err: any) {
      // console.error('Monthly AI Summary error:', err);
      setMonthlyAiError(err.message || 'Error generating summary. Please try again later.');
    } finally {
  ...
  const renderDailyReport = () => {
    ...
    const insight = getProductivityInsight(report?.productivityScore || 0);
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{report?.tasksCompleted || 0}</p>
                <p className="text-xs text-gray-500">{report?.completionRate || 0}%</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{report?.activeTasks || 0}</p>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{report?.productivityScore || 0}</p>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">{report?.tasksOverdue || 0}</p>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="text-center bg-blue-50 rounded-lg p-3">
              <div className="text-lg font-bold text-blue-600">{report?.timeEfficiency || 0}%</div>
              <div className="text-xs text-blue-700">Efficiency</div>
            </div>
            <div className="text-center bg-green-50 rounded-lg p-3">
              <div className="text-lg font-bold text-green-600">{report?.delegationRate || 0}%</div>
  ...
  const renderWeeklyReport = () => {
    const report = safeReport;
    const insight = getProductivityInsight(report?.weeklyProductivityScore || 0);
    const weekTasks = getPeriodTasks('weekly');
      ...
      <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-600">{report?.totalTasksCompleted || 0}</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-600">{report?.averageCompletionRate || 0}%</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-purple-600">{report?.weeklyProductivityScore || 0}</p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <div className="flex items-center justify-center">
              <CircularProgress 
                value={report?.weeklyProductivityScore || 0} 
                title="Weekly Score"
                color="#7c3aed"
                size={100}
              />
        ...
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-200 p-6">
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div className="bg-white bg-opacity-50 rounded-lg p-3">
              ...
              <p className="text-blue-600">{report?.averageCompletionRate || 0}% average</p>
            </div>
            <div className="bg-white bg-opacity-50 rounded-lg p-3">
              <p className="font-medium text-gray-800">Total Output</p>
              <p className="text-green-600">{report?.totalTasksCompleted || 0} tasks completed</p>
            </div>
            <div className="bg-white bg-opacity-50 rounded-lg p-3">
              <p className="font-medium text-gray-800">Consistency</p>
              <p className="text-purple-600">{report?.categoryInsights?.averageTasksPerDay || 0} tasks/day</p>
            </div>
          </div>
        </div>
        <div className="flex justify-end">
  ...
  const renderMonthlyReport = () => {
    const report = safeReport;
    const insight = getProductivityInsight(report?.monthlyProductivityScore || 0);
    const monthTasks = getPeriodTasks('monthly');
      ...
      <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-600">{report?.categoryInsights?.averageTasksPerDay || 0}</p>
              </div>
              <BarChart3 className="h-8 w-8 text-blue-600" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-purple-600">{report?.monthlyProductivityScore || 0}</p>
              </div>
              <Award className="h-8 w-8 text-purple-600" />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <div className="flex items-center justify-center">
              <CircularProgress 
                value={report?.monthlyProductivityScore || 0} 
                title="Progress"
                color="#7c3aed"
                size={90}
              />
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-2">
              {report?.weeklyBreakdown || []?.map((week: any, index: number) => {
                ...
                const weekLabel = `Week ${index + 1} (${weekStart.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} - ${weekEnd.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })})`;
